    ...
    func fetchGroups(completion: @escaping (([Group]) -> Void)) {
        ...
        GroupRepository.shared.fetchGroup(completion: { result in
            ...
            case .success(let groups):
                // todo parse icon
                completion(groups.map { Group(id: $0.pk, name: $0.name, totalCost: $0.total_cost, userBalance: $0.user_balance, icon: MoneyAppIcon.from(id: $0.icon), createDate: Date.fromISO(stringDate: $0.create_date), isFavourite: $0.is_favourite)})
